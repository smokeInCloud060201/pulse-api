...
export const EnvSelector: React.FC = () => {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '0 16px' }}>
      ...
      {showEditor && (
        <EnvEditorModal onClose={() => setShowEditor(false)} />
      )}
    </div>
  );
};

const EnvEditorModal: React.FC<{ onClose: () => void }> = ({ onClose }) => {
  ...
  const handleUpdateVar = (index: number, field: 'key' | 'value' | 'enabled', val: any) => {
    if (!selectedEnv) return;
    const newVars = [...selectedEnv.variables];
          ...
          <div style={{ width: '250px', borderRight: '1px solid var(--border-color)', display: 'flex', flexDirection: 'column' }}>
            <div style={{ padding: '12px', display: 'flex', gap: '8px' }}>
              <input 
                ...
                style={{ flex: 1, padding: '6px', background: 'var(--bg-secondary)', border: '1px solid var(--border-color)', color: 'var(--text-primary)' }}
              />
              <button onClick={handleCreate} style={{ padding: '6px 12px', background: 'var(--color-primary)', color: 'white', border: 'none', borderRadius: '4px', cursor: 'pointer' }}>+</button>
            </div>
          ...
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', background: 'var(--bg-secondary)' }}>
            {selectedEnv ? (
              <div style={{ padding: '16px', display: 'flex', flexDirection: 'column', height: '100%' }}>
                ...
                <div style={{ display: 'flex', borderBottom: '1px solid var(--border-color)', paddingBottom: '8px', marginBottom: '8px', fontWeight: 600, color: 'var(--text-muted)' }}>
                  ...
                  {selectedEnv.variables.map((v, i) => (
                    <div key={v.id} style={{ display: 'flex', gap: '8px', marginBottom: '8px', alignItems: 'center' }}>
                      ...
                      <input 
                        ...
                        style={{ flex: 1, padding: '6px', background: 'var(--bg-primary)', border: '1px solid var(--border-color)', color: 'var(--text-primary)' }}
                      />
                      <input 
                        value={v.value} 
                        ...
                        style={{ flex: 1, padding: '6px', background: 'var(--bg-primary)', border: '1px solid var(--border-color)', color: 'var(--text-primary)' }}
                      />
                      <button onClick={() => handleDeleteVar(i)} style={{ width: '40px', background: 'transparent', border: 'none', color: 'var(--color-danger)', cursor: 'pointer' }}>✕</button>
                    </div>
                  ))}
                  <button onClick={handleAddVar} style={{ marginTop: '16px', padding: '6px 12px', background: 'var(--bg-primary)', border: '1px solid var(--border-color)', color: 'var(--text-primary)', cursor: 'pointer', borderRadius: '4px' }}>
                    + Add Variable
                  </button>
                </div>
              </div>
