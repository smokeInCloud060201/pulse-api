  ...
  return (
    <div style={{ display: 'flex', height: '100%' }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', borderRight: '1px solid var(--border-color)' }}>
        <div style={{ padding: '8px', borderBottom: '1px solid var(--border-color)', fontSize: '0.85rem', color: 'var(--text-muted)' }}>
          Proto File Content
        </div>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
        <div style={{ padding: '8px', borderBottom: '1px solid var(--border-color)', display: 'flex', flexDirection: 'column', gap: '8px' }}>
          <div>
            <div style={{ fontSize: '0.8rem', color: 'var(--text-muted)', marginBottom: '4px' }}>Service Name (e.g. helloworld.Greeter)</div>
            <input 
              type="text" 
              value={request.grpc_service || ''}
              onChange={(e) => onChange({ ...request, grpc_service: e.target.value })}
              style={{ width: '100%', padding: '6px', background: 'var(--bg-primary)', border: '1px solid var(--border-color)', color: 'var(--text-primary)', borderRadius: '4px' }}
              placeholder="Package.Service"
            />
          </div>
          <div>
            <div style={{ fontSize: '0.8rem', color: 'var(--text-muted)', marginBottom: '4px' }}>Method Name (e.g. SayHello)</div>
            <input 
              type="text" 
